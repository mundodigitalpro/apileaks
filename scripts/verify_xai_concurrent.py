...
REPORT_FILE = "report_claves_xai.md"
OUTPUT_FILE = "report_valid_xai.md"
MAX_WORKERS = 10

def extract_keys(filename):
    """Extracts xAI keys from the report file."""
    keys = set()
    try:
        with open(filename, "r") as f:
            content = f.read()
            # Match keys starting with xai-
            matches = re.findall(r"xai-[a-zA-Z0-9]{48}", content)
            for key in matches:
                keys.add(key)
...
def check_key(api_key):
    """Checks the validity of an xAI API key."""
...
def main():
    ...
    report_content = "# Reporte de Verificación de Claves xAI\n\n"
