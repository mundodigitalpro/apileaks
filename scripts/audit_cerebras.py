...
OUTPUT_FILE = "report_claves_cerebras.md"
...
def search_keys(content):
    """Searches for Cerebras keys in the content."""
    # Pattern to match Cerebras keys: csk- followed by 48 alphanumeric characters
    # Example: csk-AbCd...
    pattern = re.compile(r"csk-[a-zA-Z0-9]{48}")
...
def main():
    ...
    # Filter for 'cerebras' or 'ai-key' which might contain cerebras if mislabeled, 
    # but based on previous check, 'cerebras' provider exists.
    cerebras_leaks = [leak for leak in leaks if leak.get("provider") == "cerebras"]
    print(f"Found {len(cerebras_leaks)} potential Cerebras leaks.")

    report_content = "# Reporte de Claves Cerebras Encontradas\n\n"

    for leak in cerebras_leaks:
