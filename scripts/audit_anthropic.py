...
KEYWORDS_ANTHROPIC = ["sk-ant"]
OUTPUT_FILE = "report_claves_anthropic.md"
...
def search_keys(content):
    """Searches for Anthropic keys in the content."""
    keys = []
    # Pattern to match Anthropic keys: sk-ant- followed by alfanumeric characters
...
def main():
    ...
    anthropic_leaks = [leak for leak in leaks if leak.get("provider") == "anthropic"]
    print(f"Found {len(anthropic_leaks)} potential Anthropic leaks.")

    report_content = "# Reporte de Claves Anthropic Encontradas\n\n"

    for leak in anthropic_leaks:
