...
OUTPUT_FILE = "report_claves_xai.md"
...
def search_keys(content):
    """Searches for xAI keys in the content."""
    # Pattern to match xAI keys: xai- followed by 48 alphanumeric characters
    # Example: xai-AbCd...
    pattern = re.compile(r"xai-[a-zA-Z0-9]{48}")
...
def main():
    ...
    xai_leaks = [leak for leak in leaks if leak.get("provider") == "xai"]
    print(f"Found {len(xai_leaks)} potential xAI leaks.")

    report_content = "# Reporte de Claves xAI Encontradas\n\n"

    for leak in xai_leaks:
