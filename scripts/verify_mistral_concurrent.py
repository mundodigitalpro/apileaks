...
REPORT_FILE = "report_claves_mistral.md"
OUTPUT_FILE = "report_valid_mistral.md"
MAX_WORKERS = 10

def extract_keys(filename):
    """Extracts Mistral keys from the report file."""
    keys = set()
    try:
        with open(filename, "r") as f:
            content = f.read()
            # Searching for 32 digit alphanumeric keys
            matches = re.findall(r"\b[a-zA-Z0-9]{32}\b", content)
            for key in matches:
                # Basic filter
                if any(x in key.lower() for x in ["replace", "example", "00000000", "test"]):
                    continue
                keys.add(key)
...
def check_key(api_key):
    """Checks the validity of a Mistral API key."""
...
def main():
    ...
    report_content = "# Reporte de Verificación de Claves Mistral\n\n"
