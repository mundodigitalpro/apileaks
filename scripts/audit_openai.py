...
OUTPUT_FILE = "report_claves_openai.md"
...
def search_keys(content):
    """Searches for OpenAI keys in the content."""
    # Pattern to match OpenAI keys: sk- followed by 20+ chars, potentially including 'proj-'
    # We explicitly exclude 'sk-ant-' just in case mixed content appears, though provider filter should handle it.
...
def main():
    ...
    openai_leaks = [leak for leak in leaks if leak.get("provider") == "openai"]
    print(f"Found {len(openai_leaks)} potential OpenAI leaks.")

    report_content = "# Reporte de Claves OpenAI Encontradas\n\n"

    for leak in openai_leaks:
