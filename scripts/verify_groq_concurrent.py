...
REPORT_FILE = "report_claves_groq.md"
OUTPUT_FILE = "report_valid_groq.md"
MAX_WORKERS = 10

def extract_keys(filename):
    """Extracts Groq keys from the report file."""
    keys = set()
    try:
        with open(filename, "r") as f:
            content = f.read()
            # Match keys starting with gsk_
            matches = re.findall(r"gsk_[a-zA-Z0-9]{52}", content)
            for key in matches:
                keys.add(key)
...
def check_key(api_key):
    """Checks the validity of a Groq API key."""
...
def main():
    ...
    report_content = "# Reporte de Verificación de Claves Groq\n\n"
