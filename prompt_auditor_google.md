...
# Contexto
Disponemos de un archivo de reporte de fugas (`data/leaks_api_20260214_203200.json`) que contiene metadatos sobre posibles credenciales expuestas en repositorios públicos. Necesitamos identificar específicamente las claves completas de **Google** para proceder a su revocación.

# Tu Tarea
Tu objetivo es leer el archivo de leakes, filtrar los casos de Google, acceder a los archivos fuente originales y extraer las API Keys completas que han sido expuestas.
...
2.  **Filtrar por Proveedor**:
    - Procesa el JSON y selecciona únicamente los objetos donde el campo `provider` sea igual a `"google"`.
...
3.  **Recuperación y Extracción (Iterar sobre los filtrados)**:
    - Para cada entrada identificada:
        ...
        c. **Buscar la Clave**: Analiza el texto descargado buscando patrones de API Keys de Google.
           - **CRÍTICO**: Las claves de Google (incluyendo Gemini/Vertex AI) suelen comenzar con `AIza`.
           - Busca cadenas que coincidan con este patrón (ej. `AIzaSy...`).
           - Puedes usar una expresión regular como `AIza[0-9A-Za-z\-_]{35}` para capturar la clave completa (suelen tener 39 caracteres).
...
4.  **Generar Reporte**:
    - Crea un archivo de salida llamado `reporte_claves_google.md` (o imprime en consola si se solicita) con la siguiente estructura para cada clave encontrada:
    ...
    - **Clave Expuesta**: `AIza...................................` (La clave completa)
    ```
